...
% This m file always ahs 2 neurons using a different motherspike
...
function [MOV_REB,ALL_REB] = ...
    TC_model_CX_SNr_cond_changed_parfor_opt_specifyneurons...
                                          ...
                                          mov_onset,...
                                          deg_of_jit,...
                                          num_trials,dir_name_trace,S,FG_SNR,neur)
...
for tr_ind = 1:num_trials
    ...
    if (numel(F_SNr) > 1)
        ...
        stimulated_spikes = spike_times(neur-FG_SNR(end)+1:sum(FG_SNR),:);
    ...
    % we now add the other 2 neurons
    [spike_times] = MIP_imp_v4_beta(deg_of_jit,30-neur,max(F_SNr),T(T<=mov_onset));
    all_spikes(3).spikes = spike_times;
