...
function [dir_name] = TCmodel_arrayjob_big_parallel(job_id, num_jobs)
...
rebound_spk = zeros(9600/num_jobs,num_trials); %Result vector, rebound spikes after decrease
all_reb_spk = zeros(9600/num_jobs,num_trials); %Result vector, (check how its done may be the whole simuation)
...
parfor exp = 1:size(NT_GS_JV_TF,2)
    ...
    [F_SNr, FG_SNR] = par_obtain(nr_neurons_increase(exp), freq_increase(exp), N_SNr);
...
for i = 1:length(dir_name_list)
    ...
    [F_SNr, FG_SNR] = par_obtain(nr_neurons_increase(i), freq_increase(i), N_SNr);
