...
function [dir_name] = TCmodel_arrayjob_specifyneuroapart_big_parallel(neur, job_id, num_jobs)

% in this experiment deletion method is used but
% specified neurons are always "apart" using a different motherspike
% difference is small, but prefer to have different files to avoid
% confusion

% IMPORTANT: NEUR is the number of experiments for deletion, so 30 - neur =
% number of neurons in diffmoth mode
...
nr_neurons = 1:1:neur; %nr of neurons with firing rate increase
...
rebound_spk = zeros((size(ALL_EXPERIMENTS,2))/num_jobs,num_trials); %Result vector, rebound spikes after decrease
all_reb_spk = zeros((size(ALL_EXPERIMENTS,2))/num_jobs,num_trials); %Result vector, (check how its done may be the whole simuation)
...
for exp = 1:size(NT_GS_JV_TF,2)
    ...
    [F_SNr, FG_SNR] = par_obtain(nr_neurons_increase(exp), freq_increase(exp), neur);
    
    SPK = [];
    
    exppath = ['ICEBERG_DELET_' , num2str(30-neur) ,'NEURONS/FREQINCTO_',num2str(freq_increase(exp))];
    ...
    switch sim_mode %for now we only have sim mode 1 and 2
        ...
        case 3
            ...
            [rebound_spk(exp,:),all_reb_spk(exp,:)] = ...
                TC_model_CX_SNr_cond_changed_parfor_opt_specifyneurons(...
                F_SNr,0,G_SNr(exp),...
                T,mov_onset,jit_val(exp),...
                num_trials,dir_name_trace,exp,FG_SNR,neur);
...
for i = 1:length(freq_increase)
    ...
    exppath = ['ICEBERG_DELET_' , num2str(30-neur) ,'NEURONS/FREQINCTO_',num2str(freq_increase(i))];
    freq_experiment = fullfile(pwd, exppath);
    ...
    [F_SNr, FG_SNR] = par_obtain(nr_neurons_increase(i), freq_increase(i), neur);
