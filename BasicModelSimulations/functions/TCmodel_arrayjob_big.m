...
function [dir_name] = TCmodel_arrayjob_big(job_id, num_jobs)
...
for exp = 1:size(NT_GS_JV_TF,2)
    ...
    save([dir_name_cp 'corr-' num2str(jit_val(exp)*100) '-freq_inc-' num2str(freq_increase(exp)) '-nSNr_inc-' num2str(nr_neurons_increase(exp)) '-job-' num2str(job_id)],...
