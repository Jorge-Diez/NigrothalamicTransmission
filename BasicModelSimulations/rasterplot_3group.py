...
nr_neurons_inq = int(input("Indicate how many neurons are in group with freq. increase: "))
nr_neurons_dec = int(input("Indicate how many neurons are in group with freq. decrease: "))
...
for i in range(len(np.arange(0,1000,firing_rate_bin))-1):
    ...
    baseline_spike_bits_slice = spike_bits[0:30-(nr_neurons_inq+nr_neurons_dec),i*jumps:(i+1)*jumps]
    freqdec_spike_bits_slice = spike_bits[30-(nr_neurons_inq+nr_neurons_dec):30-nr_neurons_inq,i*jumps:(i+1)*jumps]
    freqinc_spike_bits_slice = spike_bits[30-nr_neurons_inq:30,i*jumps:(i+1)*jumps]
    
    
    all_nr_spikes = np.count_nonzero(all_spike_bits_slice)
    baseline_nr_spikes = np.count_nonzero(baseline_spike_bits_slice)
    freqdec_nr_spikes = np.count_nonzero(freqdec_spike_bits_slice)
    ...
    freqdec_firing_rate.append(freqdec_nr_spikes)
...
freqdec_firing_rate_np =  np.asarray(freqdec_firing_rate, dtype=np.float32)    
...
baseline_firing_rate = (baseline_firing_rate_np * (1000/firing_rate_bin)) / (30-(nr_neurons_inq+nr_neurons_dec))
freqdec_firing_rate =  (freqdec_firing_rate_np * (1000/firing_rate_bin)) / nr_neurons_dec
freqinc_firing_rate =  (freqinc_firing_rate_np * (1000/firing_rate_bin)) / nr_neurons_inq
...
freqdec_convolved_firing_rate = np.convolve( freqdec_firing_rate , gauss_filter, mode='same'  )
freqinc_convolved_firing_rate = np.convolve( freqinc_firing_rate , gauss_filter, mode='same'  )



fig, axs = plt.subplots(4, sharex=True, sharey=True, num=8, figsize = (19,10))
...
axs[2].plot(np.arange(0,1000,firing_rate_bin) , freqdec_convolved_firing_rate)
axs[3].plot(np.arange(0,1000,firing_rate_bin) , freqinc_convolved_firing_rate)

axs[0].set_title("Firing rate of all neurons")
axs[1].set_title("Firing rate of baseline neurons")
axs[2].set_title("Firing rate of freqdec neurons")
axs[3].set_title("Firing rate of freqinc neurons")
...
plt.plot(  [0,30] , [30-(nr_neurons_inq+nr_neurons_dec)-0.5,30-(nr_neurons_inq+nr_neurons_dec)-0.5], 'k'   )
plt.plot(  [30-(nr_neurons_inq+nr_neurons_dec)-0.5,30-(nr_neurons_inq+nr_neurons_dec)-0.5] , [-0.5,30] , 'k'  )

plt.plot(  [0,30] , [30-nr_neurons_inq-0.5,30-nr_neurons_inq-0.5], 'k'   )
plt.plot(  [30-nr_neurons_inq-0.5,30-nr_neurons_inq-0.5] , [-0.5,30] , 'k'  )
...
corr_matrix = np.corrcoef(spike_bits_binned[0:30-(nr_neurons_inq+nr_neurons_dec)])
upper_mask =  np.tri(corr_matrix.shape[0], k=-1)
upper_mask[np.diag_indices(upper_mask.shape[0])] = 1
upper_corr_matrix = np.ma.array(corr_matrix, mask=upper_mask)
...
#FREQ DECREASE
corr_matrix = np.corrcoef( spike_bits_binned[30-(nr_neurons_inq+nr_neurons_dec):30-nr_neurons_inq]   )
upper_mask =  np.tri(corr_matrix.shape[0], k=-1)
upper_mask[np.diag_indices(upper_mask.shape[0])] = 1
upper_corr_matrix = np.ma.array(corr_matrix, mask=upper_mask)
...
masked_zeroed_corr_matrix = zero_mask.filled()
#calculate means
freqdec_group_mean= np.sum(masked_zeroed_corr_matrix)/ np.count_nonzero(masked_zeroed_corr_matrix)


#plot and save
plt.figure(23, figsize=(10,8))
plt.matshow(upper_corr_matrix, fignum=23)
plt.colorbar()
plt.title("Correlation matrix of frequency decrease spiketrains for " + exp_name + " with bins of " + str(corr_bin) + " ms" + "\n" + "Mean of freqdec: " + str(freqdec_group_mean))

plt.savefig(RESULTS_FOLDER + "\\" + "Correlation_Matrix_freqdec",bbox_inches='tight')
...
corr_matrix = np.corrcoef(spike_bits_binned[30-nr_neurons_inq:30])
upper_mask =  np.tri(corr_matrix.shape[0], k=-1)
upper_mask[np.diag_indices(upper_mask.shape[0])] = 1
upper_corr_matrix = np.ma.array(corr_matrix, mask=upper_mask)
...
CV_baseline = CV[0:30-(nr_neurons_inq+nr_neurons_dec)]
CV_freqdec = CV[30-(nr_neurons_inq+nr_neurons_dec):30-nr_neurons_inq]
CV_freqinc = CV[30-nr_neurons_inq:30]


CV_baseline_mean = np.mean(CV_baseline)
CV_freqdec_mean = np.mean(CV_freqdec)
CV_freqinc_mean = np.mean(CV_freqinc)
CV_both_groups = np.mean(CV)



CV_to_plot = [CV_baseline_mean, CV_freqdec_mean, CV_freqinc_mean, CV_both_groups]
CV_labels = ["Baseline", "Freq Decrease", "Freq Increase", "All neurons"]
CV_err = [np.std(CV_baseline), np.std(CV_freqdec), np.std(CV_freqinc), np.std(CV)]


plt.figure(16, figsize=(10,9))
plt.title("ISI CV Means")
plt.bar(range(4), CV_to_plot, tick_label=CV_labels, width=0.6, edgecolor='k', yerr=CV_err)
