...
#depends on the type of experiment, basically made for delet method with specific neurons in diffmoth mode
...
ALL_MEAN_OVR = np.zeros((nr_neurons,nr_experiments))
ALL_MEAN_OVR_DIFFERENCE = np.zeros((nr_neurons,nr_experiments))

#Im doing this with a migraine so i want to make it simple
ALL_OVR_CORR_30 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_40 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_50 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_60 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_70 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_80 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_90 = np.zeros((nr_neurons,nr_experiments))
ALL_OVR_CORR_100 = np.zeros((nr_neurons,nr_experiments))
...
for i, experiment_name in enumerate(os.listdir(main_big_exp_folder)):
    ...
    OVR_RESULTS = np.zeros((nr_neurons,8)) #all TQ results will be pooled here
    OVR_RESULTS_DIFFERENCE = np.zeros((nr_neurons,8)) #TQ diferrence with respect to baseline
